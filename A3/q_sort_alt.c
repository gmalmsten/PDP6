...
int main(int argc, char *argv[])
{
    ...
    int n1 = chunks[rank], n_recv, n_send;             // Size of sub list and elements to receive
    ...
    for(int iter = 0; iter < max_iter; iter++)
    {
        ...
        // Determine elements to receive  // Need to decide how to sendrecv, this is wrong
        if(rank < limit)
        {
            n2 = split_index + 1;
        }
        ...
        MPI_Sendrecv(&split_index, 1, MPI_INT, friend, rank, &friend_split, 1, MPI_INT, friend, friend, MPI_COMM_WORLD, &status);
        if(rank < limit)
        {
            temp_list = (int *)malloc((split_index+1)*sizeof(int));
            memcpy(temp_list, local_list, (split_index+1)*sizeof(int));
            local_list = (int *)realloc(local_list, (split_index + n2)*sizeof(int));

        }
        else
        {
            temp_list = (int *)malloc((n1 - split_index)*sizeof(int));
            memcpy(temp_list, local_list, (split_index+1)*sizeof(int));
            local_list = (int *)realloc(local_list, (n1 - split_index + n2)*sizeof(int));
        }
        ...
        print_list(local_list, n1, rank);

        // Split communicator and iterate again
    }


    // Gather all local lists
    MPI_Gather(&n1, 1, MPI_INT, chunks, 1, MPI_INT, 0, MPI_COMM_WORLD);
    ...
    MPI_Gatherv(local_list, n1, MPI_INT, global_list, chunks, displs, MPI_INT, 0, MPI_COMM_WORLD);
    
    if(rank == 0)print_list(global_list, n, rank);
